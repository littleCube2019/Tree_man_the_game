    ...
    for(var i =0 ; i < combat_report.length ; i++){
      ...
      if(r["wall_damaged"]>0){
        ...
        if(r["location"]>0){
          ...
          var msg =  "位於"+"<b>" + direct_dic[r["direction"]] + "方</b>城門下方發生戰爭<br>我方造成"+r["army_attack"]+"點傷害，樹人造成"+r["enemy_attack"]+"點傷害<br>先鋒部隊剩餘血量為:"+r["army_hp"]+"，該樹人剩下"+r["enemy_hp"]+"點血量<br>"; 
          reports.push(msg);
        }
...
exports.combat = function(Env, dir, total_report, defender_data){
    ...
    var army_attack = {"armor":0, "archer":0, "ranger":0};
    ...
    if(nearest_enemy!=-1){
      ...
      */
	  ...
	  total_damage += defence_attack
	  total_damage = total_damage * Env.morale
    }
